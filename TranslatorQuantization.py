...
class Quantization:
    ...
    def F32编码Q8_K_X(Self, 输入数组):
        ...
        缩放因子编码 = Self.F32编码F16_E0M15(缩放因子)
    ...
    def Q8_K_X解码F32(Self, 量化数据, 缩放因子编码, 零点数组, 原始形状):
        ...
        缩放因子 = Self.F16_E0M15解码F32(缩放因子编码).astype(np.float32)
    ...
    def Q2_K_X解码F32(Self, 打包数据, 缩放因子编码, 零点数组, 原始形状):
        ...
        反量化扁平 = 反量化块.flatten()[:总元素数]
        return 反量化扁平.reshape(原始形状)
    def F32编码BF16(Self, 输入数组): #Qwen3-Max编写
        输入数组 = np.asarray(输入数组, dtype=np.float32)
    ...
    def BF16解码F32(Self, 编码数组): #Qwen3-Max编写
    ...
    def F32编码F16_E0M15(Self, 浮点数组, desc="tqdm.vectors.quantization"): #Qwen3-Max编写
        return np.round(np.clip(浮点数组, -1.0, 1.0 - 2**-15) * 32768.0).astype(np.int16).view(np.uint16)
    def F16_E0M15解码F32(Self, 编码数组): #Qwen3-Max编写
        return 编码数组.view(np.int16).astype(np.float32) / 32768.0
