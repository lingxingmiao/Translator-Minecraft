from TranslatorLib import re, os, time, json, random, pickle, zipfile, Path, ConfigFactory, HOCONConverter, eb
        ...
        try:
            for _ in tqdm(range(1), desc="tqdm.file.read"): 
                with zipfile.ZipFile(file_path, 'r') as f:
                    可用文件列表 = [False]
                    文件列表 = f.namelist()
                    文件路径 = f"{cache_path}/{Self.随机16进制字符串(1)}_{Path(file_path).stem}"
            ...
            Self.写入日志("log.module.zip.read.error", e=e, info_level=3)
            raise FileNotFoundError(eb.format_exc())
        return 可用文件列表
    def 读取资源文件(Self, file0: str, file1: str = ""):
        压缩路径 = ""
        文件1 = ""
        file2 = ""
        if Path(file0).suffix in [".zip", ".jar"]:
            缓存路径 = f"{Self.Config.PATH_CACHE}/{Self.随机16进制字符串(4)}"
