...
- 计算加速器(可选)：NVIDIA支持CUDA Toolkit 12.0的Maxwell2.0以上架构 8GB内存
- 内存(程序占用)：最低4GB 推荐8GB
- 存储：平均4GB（按向量大小）
...
```
## 推荐模型翻译速度排名 16GRAM
- [LiquidAI/LFM2-8B-A1B](https://huggingface.co/LiquidAI/LFM2-8B-A1B)（单次多次翻译可能导致输出问题）（LMStudio并行推理会炸）
- [LiquidAI/LFM2-24B-A2B](https://huggingface.co/LiquidAI/LFM2-24B-A2B)（单次多次翻译可能导致输出问题）（LMStudio并行推理会炸）
- [MoonshotAI/Kimi-VL-A3B-Instruct](https://huggingface.co/moonshotai/Kimi-VL-A3B-Instruct)（可能导致无限输出）（LMStudio并行推理会炸）
- [Qwen/Qwen3-30B-A3B-Instruct-2507](https://huggingface.co/Qwen/Qwen3-30B-A3B-Instruct-2507)（速度质量平衡）
- [Google/Gemma-4-26B-A4B-it](https://huggingface.co/google/gemma-4-26B-A4B-it)（速度质量平衡）
